  ...
  async handleDosOrgSync(
    ...
    switch (payload.event) {
      ...
      case 'org.created': {
        ...
        const ownerEmail = payload.data.owner_email;

        if (isNonEmptyString(orgName) && isNonEmptyString(ownerEmail)) {
          this.logger.log(
            `Org created: ${orgName} for owner ${ownerEmail}`,
          );
        }
        break;
      }
      ...
      case 'org.updated': {
        ...
        if (isNonEmptyString(orgId) && isNonEmptyString(orgName)) {
          const workspace = await this.workspaceRepository.findOne({
            where: [{ id: orgId }, { displayName: orgName }],
          });
          ...
          if (isDefined(workspace)) {
            ...
            this.logger.log(`Updated workspace ${workspace.id} name to ${orgName}`);
          }
        }
        break;
      }
      ...
      case 'org.member_added': {
        ...
        const orgId = payload.data.org_id;
        const orgName = payload.data.org_name;
        ...
        if (isNonEmptyString(userEmail)) {
          ...
          if (isDefined(user) && isDefined(workspace)) {
            ...
            this.logger.log(`Added user ${userEmail} to workspace ${workspace.id}`);
          }
        }
        break;
      }
      ...
      case 'user.updated': {
        const userEmail = payload.data.email?.toLowerCase() || payload.data.user_email?.toLowerCase();
